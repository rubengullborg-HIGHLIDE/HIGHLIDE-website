-- Small, field-specific Danish/English lookup. Each row is one synonym group.
-- Terms are lowercase single tokens; hyphenated words such as t-shirts stay intact.
-- Keep related categories/shades separate: jeans != trousers, cream != white.
create table public.search_synonym_groups (
  kind text not null check (kind in ('color', 'category')),
  group_key text not null,
  terms text[] not null,
  primary key (kind, group_key),
  check (array_ndims(terms) = 1 and cardinality(terms) between 2 and 20),
  check (array_position(terms, null) is null),
  check (
    array_to_string(terms, '|') ~ '^[[:lower:][:digit:]æøå-]+([|][[:lower:][:digit:]æøå-]+)+$'
  )
);

comment on table public.search_synonym_groups is
  'Shared, read-only search vocabulary. Edit lowercase single-token terms here to add Danish/English equivalents without reimporting products.';

alter table public.search_synonym_groups enable row level security;
revoke all on table public.search_synonym_groups from public, anon, authenticated;
grant select on table public.search_synonym_groups to authenticated;

-- Vocabulary is shared catalogue configuration, not user-owned data.
create policy "Authenticated users can read search synonyms"
  on public.search_synonym_groups
  for select
  to authenticated
  using (true);

insert into public.search_synonym_groups (kind, group_key, terms) values
  ('color', 'white', array['hvid', 'hvidt', 'hvide', 'white']),
  ('color', 'black', array['sort', 'sorte', 'black']),
  ('color', 'blue', array['blå', 'blåt', 'blue']),
  ('color', 'grey', array['grå', 'gråt', 'grey', 'gray']),
  ('color', 'green', array['grøn', 'grønt', 'grønne', 'green']),
  ('color', 'brown', array['brun', 'brunt', 'brune', 'brown']),
  ('color', 'red', array['rød', 'rødt', 'røde', 'red']),
  ('color', 'yellow', array['gul', 'gult', 'gule', 'yellow']),
  ('color', 'pink', array['lyserød', 'lyserødt', 'lyserøde', 'rosa', 'pink']),
  ('color', 'purple', array['lilla', 'purple']),
  ('color', 'navy', array['marineblå', 'marineblåt', 'navy']),
  ('color', 'turquoise', array['turkis', 'turkist', 'turkise', 'turquoise']),
  ('color', 'burgundy', array['bordeaux', 'vinrød', 'vinrødt', 'vinrøde', 'burgundy']),
  ('color', 'cream', array['creme', 'cremefarvet', 'cream']),
  ('color', 'multicolor', array['flerfarvet', 'flerfarvede', 'multifarvet', 'multi', 'multicolor', 'multicolour']),
  ('category', 'trousers', array['buks', 'bukser', 'pant', 'pants', 'trouser', 'trousers']),
  ('category', 'shirts', array['skjorte', 'skjorter', 'shirt', 'shirts']),
  ('category', 't-shirts', array['t-shirt', 't-shirts', 'tshirt', 'tshirts', 'tee', 'tees']),
  ('category', 'jackets', array['jakke', 'jakker', 'jacket', 'jackets']),
  ('category', 'coats', array['frakke', 'frakker', 'coat', 'coats']),
  ('category', 'knitwear', array['strik', 'knit', 'knitwear']),
  ('category', 'sweaters', array['striktrøje', 'striktrøjer', 'sweater', 'sweaters', 'pullover', 'pullovers']),
  ('category', 'hoodies', array['hættetrøje', 'hættetrøjer', 'hoodie', 'hoodies']),
  ('category', 'sweatshirts', array['sweatshirt', 'sweatshirts']),
  ('category', 'cardigans', array['cardigan', 'cardigans']),
  ('category', 'vests', array['vest', 'veste', 'vests', 'waistcoat', 'waistcoats']),
  ('category', 'blazers', array['blazer', 'blazere', 'blazers']),
  ('category', 'shoes', array['sko', 'shoe', 'shoes']),
  ('category', 'sneakers', array['kondisko', 'sneaker', 'sneakers']),
  ('category', 'boots', array['støvle', 'støvler', 'boot', 'boots']),
  ('category', 'sandals', array['sandal', 'sandaler', 'sandals']),
  ('category', 'socks', array['sok', 'sokker', 'strømpe', 'strømper', 'sock', 'socks']),
  ('category', 'skirts', array['nederdel', 'nederdele', 'skirt', 'skirts']),
  ('category', 'dresses', array['kjole', 'kjoler', 'dress', 'dresses']),
  ('category', 'jeans', array['cowboybukser', 'jeans']),
  ('category', 'tops', array['top', 'toppe', 'tops']),
  ('category', 'overshirts', array['overskjorte', 'overskjorter', 'overshirt', 'overshirts']);
...
tokens as materialized (
  -- Expand alternatives here, then keep ONE row per original query token.
  -- Matching several synonyms must not inflate coverage or relevance.
  select
    qt.token,
    coalesce(
      array_agg(distinct aliases.term) filter (where g.kind = 'color'),
      '{}'::text[]
    ) as color_terms,
    coalesce(
      array_agg(distinct aliases.term) filter (where g.kind = 'category'),
      '{}'::text[]
    ) as category_terms
  from query_tokens as qt
  left join public.search_synonym_groups as g on qt.token = any(g.terms)
  left join lateral unnest(g.terms) as aliases(term) on true
  group by qt.token
),
normalized as materialized (
  select
    ...
    regexp_split_to_array(
      lower(coalesce(f.color, '') || ' ' || coalesce(f.color_group, '')),
      '[^[:alnum:]æøå-]+'
    ) as color_words,
    regexp_split_to_array(
      lower(coalesce(f.category, '') || ' ' || coalesce(f.product_type, '')),
      '[^[:alnum:]æøå-]+'
    ) as category_words
  from filtered as f
),
token_scores as materialized (
  select
    ...
    greatest(
      ...
      case
        when fuzzy.similarity >= 0.55 then fuzzy.similarity * 4.0
        else 0.0
      end
    ) as token_score
  ...
  cross join lateral (
    select greatest(
      -- Exact vocabulary matches get the same weight as an original word
      -- in that field. Never translate brand/name/description matches.
      case when n.category_words && t.category_terms then 7.0 else 0.0 end,
      case when n.color_words && t.color_terms then 6.0 else 0.0 end,
      case
  ...
  cross join lateral (
    select case
      when scores.lexical_score = 0 and char_length(t.token) >= 4
        then public.strict_word_similarity(
          t.token,
          concat_ws(
            ' ',
            n.name_text,
...
product_scores as (
  ...
  group by
    ...
    n.tags_text, n.description_text, n.store_text, n.color_words,
    n.category_words, q.phrase
),
ranked as (
  select
    ps.*,
...
comment on function public.search_products_ranked(
  ...
  'Ranks multi-word product searches using field weights, Danish/English color and category synonyms, and pg_trgm typo tolerance.';
